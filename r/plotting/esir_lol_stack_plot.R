...
path_to_lockdown_output_files <- "/Volumes/tiny/projects/covid/science_revision/data/lol/"
...
get_pred <- function(x, r_0 = 2) {
  ...
  tmp_4wk_casename <- glue("{tmp_date}_lol_r{r_0}_4wk_{x}")
  tmp_6wk_casename <- glue("{tmp_date}_lol_r{r_0}_6wk_{x}")
  tmp_8wk_casename <- glue("{tmp_date}_lol_r{r_0}_8wk_{x}")
  
  tmp_4wk_pred <- fread(glue("{path_to_lockdown_output_files}{tmp_4wk_casename}_data.txt"))[, date := as.Date(date)][date >= as.Date(x, "%Y%m%d")][, `:=` (scenario = "4 weeks", start_date = as.Date(x, "%Y%m%d"))][]
  tmp_6wk_pred <- fread(glue("{path_to_lockdown_output_files}{tmp_6wk_casename}_data.txt"))[, date := as.Date(date)][date >= as.Date(x, "%Y%m%d")][, `:=` (scenario = "6 weeks", start_date = as.Date(x, "%Y%m%d"))][]
  tmp_8wk_pred <- fread(glue("{path_to_lockdown_output_files}{tmp_8wk_casename}_data.txt"))[, date := as.Date(date)][date >= as.Date(x, "%Y%m%d")][, `:=` (scenario = "8 weeks", start_date = as.Date(x, "%Y%m%d"))][]
...
# save output ----------
ggsave(filename = glue("fig/{tmp_outname}"),
       plot     = full_plot,
       height   = 10,
       width    = 10,
