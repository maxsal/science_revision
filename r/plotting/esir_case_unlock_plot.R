...
path_to_lockdown_output_files <- "/Volumes/tiny/projects/covid/science_revision/data/early_lockdown/"

# load data ----------
obs <- fread("data/covid19india_national_counts_20211031.csv")[date >= start_date]

scen_dates <- c("2021-03-13", "2021-03-19", "2021-03-30", "2021-04-15")

for (i in seq_along(scen_dates)) {
  tmp_filename <- glue("{scen_dates[i]}_t4_data.txt")
  if (i == 1) {
...
unlock_plot <- tsp[data.table::between(date, start_date, end_date)] %>%
  ...
  labs(
    ...
    caption  = glue("**Notes:** Observations and prediction period until {format(end_date, '%B %e, %Y')}. ",
                    "Solid vertical lines indicate lockdown start date and dashed vertical lines indicate lockdown end date. ",
