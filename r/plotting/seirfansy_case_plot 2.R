...
# obs <- fread("https://raw.githubusercontent.com/maxsal/science_revision/main/data/covid19india_national_counts_20211031.csv")[, date := as.Date(date)][date >= start_date][]

t2   <- fread("seirfansy_data/Tier2.csv")[, .(N = V1, daily_cases = value, date = dates)][, date := as.Date(date)][N > 100][, scenario := "Tier 2 - February 19"]
t3   <- fread("seirfansy_data/Tier3.csv")[, .(N = V1, daily_cases = value, date = dates)][, date := as.Date(date)][N > 100][, scenario := "Tier 3 - March 13"]
t4_1 <- fread("seirfansy_data/Tier4_1.csv")[, .(N = V1, daily_cases = value, date = dates)][, date := as.Date(date)][N > 100][, scenario := "Tier 4 - March 19"]
t4_2 <- fread("seirfansy_data/Tier4_2.csv")[, .(N = V1, daily_cases = value, date = dates)][, date := as.Date(date)][N > 100][, scenario := "Tier 4 - March 30"]
t4_3 <- fread("seirfansy_data/Tier4_3.csv")[, .(N = V1, daily_cases = value, date = dates)][, date := as.Date(date)][N > 100][, scenario := "Tier 4 - April 15"]
...
(cases_p <- tps[data.table::between(date, start_date, end_date)][, lt := "solid"][scenario == "Tier 4 - March 30", lt := "longdash"][scenario == "Tier 4 - April 15", lt := "dotted"][] %>%
  ...
  size = 3.5, hjust = c(0, 1, 1, 0, 0, 0), show.legend = FALSE) +
    ...
    labs(title    = "SEIRfansy predicted number of daily COVID-19 cases under various interventions",
...
# save output ----------
ggsave(filename = here("fig", "seirfansy", "seirfansy_case_plot.pdf"),
       plot     = cases_p,
       height   = 5,
       width    = 15,
       units = "in", device = cairo_pdf)

ggsave(filename = here("fig", "seirfansy", "seirfansy_case_plot.png"),
       plot     = cases_p,
       height   = 5,
       width    = 15,
