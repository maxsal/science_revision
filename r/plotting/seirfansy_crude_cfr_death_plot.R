...
t2   <- fread("seirfansy_data/Tier2.csv")[, .(N = V1, daily_deaths = value, date = dates)][, date := as.Date(date)][N > 100][, scenario := "Tier 2 - February 19"]
t3   <- fread("seirfansy_data/Tier3.csv")[, .(N = V1, daily_deaths = value, date = dates)][, date := as.Date(date)][N > 100][, scenario := "Tier 3 - March 13"]
t4_1 <- fread("seirfansy_data/Tier4_1.csv")[, .(N = V1, daily_deaths = value, date = dates)][, date := as.Date(date)][N > 100][, scenario := "Tier 4 - March 19"]
t4_2 <- fread("seirfansy_data/Tier4_2.csv")[, .(N = V1, daily_deaths = value, date = dates)][, date := as.Date(date)][N > 100][, scenario := "Tier 4 - March 30"]
t4_3 <- fread("seirfansy_data/Tier4_3.csv")[, .(N = V1, daily_deaths = value, date = dates)][, date := as.Date(date)][N > 100][, scenario := "Tier 4 - April 15"]
...
full_plt <- patched +
  plot_annotation(
    ...
    caption  = glue("**Notes:** Observations and prediction period until {format_date(end_date)}. ",
                    "Figures in boxes show peak number of deaths for each intervention.<br>SEIRfansy predicted cases reported are multipled by the given CFR schedule.<br>",
...
# save output ----------
ggsave(filename = here("fig", "seirfansy_crude_cfr_death_plot.pdf"),
       plot     = full_plt,
       height   = 12,
       width    = 15,
